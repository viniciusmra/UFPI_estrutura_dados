...
public class VectorST<Key, Value> {
	Vector<Key> keys ;
	Vector<Value> values ;  // symbol table values
    private int n = 0;      // number of elements in symbol table


	public VectorST(){
		 this.keys = new Vector<Key>(); // symbol table keys
		 this.values = new Vector<Value>();  // symbol table values
	}
