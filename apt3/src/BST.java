...
public class BST<Key extends Comparable<Key>, Value> {
    ...
    protected class Node {
        ...
        public Node(Key key, Value val, int size, Node anterior) {
            this.key = key;
            this.val = val;
            this.size = size;
    ...
    //    
    //    return getNode(key).size;
    //}  
  
    // Vinicius
    // Método que encontra a profundidade de um dado elemento
   /* public int getDepth(Key key){
        Node x = root;
        int depth = 0;
        while(true){
            // caso o nó esteja vazio, retorna -1
            if(x == null){ 
                return -1;
            }
            if(x.key.compareTo(key) == 0){ // caso a chave do nó seja a que está sendo buscada, retorna a profundidade
                return depth;
